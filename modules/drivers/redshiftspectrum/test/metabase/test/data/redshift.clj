(ns metabase.test.data.redshiftspectrum
...
(doseq [[base-type database-type] {:type/BigInteger "BIGINT"
                                   ...
                                   :type/Text       "TEXT"}]
  (defmethod sql.tx/field-base-type->sql-type [:redshiftspectrum base-type] [_ _] database-type))

;; If someone tries to run Time column tests with Redshift give them a heads up that Redshift does not support it
(defmethod sql.tx/field-base-type->sql-type [:redshiftspectrum :type/Time]
  [_ _]
  (throw (UnsupportedOperationException. "Redshift does not have a TIME data type.")))

(def ^:private db-connection-details
  (delay {:host     (tx/db-test-env-var-or-throw :redshiftspectrum :host)
          :port     (Integer/parseInt (tx/db-test-env-var-or-throw :redshiftspectrum :port "5439"))
          :db       (tx/db-test-env-var-or-throw :redshiftspectrum :db)
          :user     (tx/db-test-env-var-or-throw :redshiftspectrum :user)
          :password (tx/db-test-env-var-or-throw :redshiftspectrum :password)}))

(defmethod tx/dbdef->connection-details :redshiftspectrum
  [& _]
...
(defmethod sql-jdbc.sync/excluded-schemas :redshiftspectrum [_]
  (excluded-schemas))

(defmethod sql.tx/create-db-sql         :redshiftspectrum [& _] nil)
(defmethod sql.tx/drop-db-if-exists-sql :redshiftspectrum [& _] nil)

(defmethod sql.tx/pk-sql-type :redshiftspectrum [_] "INTEGER IDENTITY(1,1)")

(defmethod sql.tx/qualified-name-components :redshiftspectrum [& args]
...
(defmethod ddl/drop-db-ddl-statements :redshiftspectrum
  [& args]
  (apply (get-method ddl/drop-db-ddl-statements :sql-jdbc/test-extensions) args))

(defmethod sql.tx/drop-table-if-exists-sql :redshiftspectrum
  [& args]
